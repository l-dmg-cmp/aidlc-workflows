...
Invoke-WebRequest `
  -Uri https://github.com/l-dmg-cmp/aidlc-workflows/releases/latest/download/install.ps1 `
  -OutFile $installer
& $installer
...
verification. It defaults to `l-dmg-cmp/aidlc-workflows`.
...
```bash
tmp="$(mktemp -d)"
tag="$(gh release view --repo l-dmg-cmp/aidlc-workflows --json tagName --jq .tagName)"
gh release download "$tag" --repo l-dmg-cmp/aidlc-workflows --dir "$tmp" \
...
gh attestation verify "$tmp/install.sh" \
  ...
  --repo l-dmg-cmp/aidlc-workflows \
  --signer-workflow l-dmg-cmp/aidlc-workflows/.github/workflows/release.yml \
...
```bash
gh release download v2.5.45 --repo l-dmg-cmp/aidlc-workflows --dir ./aidlc-offline
```
...
source_repo="${AIDLC_RELEASE_REPOSITORY:-l-dmg-cmp/aidlc-workflows}"
