...
1. The engine creates `memory.md` from
   `.aicockpit/knowledge/aidlc-shared/memory-template.md` when it emits the
   ...
   command: `mkdir -p "$(dirname "<memory_path>")" && { [ -f "<memory_path>" ] || cp ".aicockpit/knowledge/aidlc-shared/memory-template.md" "<memory_path>"; }`.
