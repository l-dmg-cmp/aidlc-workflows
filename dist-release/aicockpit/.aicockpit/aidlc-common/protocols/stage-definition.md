...
under `.aicockpit/aidlc-common/stages/`. The schema (`stage-schema.ts`), the
...
`.aicockpit/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.aicockpit/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.aicockpit/tools/data/stage-graph.json`. Consumers continue to read the compiled
