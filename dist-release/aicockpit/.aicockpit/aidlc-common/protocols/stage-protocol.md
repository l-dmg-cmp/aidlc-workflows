...
`.aicockpit/aidlc-common/protocols/stage-protocol-construction.md`.
...
State and audit updates use the CLI tools in `.aicockpit/tools/`. These tools handle atomic read-modify-write, timestamp generation, and audit formatting internally. Do NOT use Edit or Write for these updates — those tools show diffs that create visual noise.
...
2. `.aicockpit/knowledge/aidlc-shared/` — shared methodology principles
3. `.aicockpit/knowledge/[agent-name]/` — agent-specific methodology
...
`.aicockpit/aidlc-common/protocols/stage-protocol-ensemble.md`.
...
| **Scope** | Controls which stages execute and at what depth. Eleven built-in scopes, one file per scope under `.aicockpit/scopes/aidlc-<name>.md`: enterprise, feature, mvp, poc, bugfix, refactor, infra, security-patch, classic, workshop, express. Custom scopes can be added without editing this file. |
...
`.aicockpit/aidlc-common/protocols/stage-protocol-reviewer.md`.
...
- `.aicockpit/sensors/aidlc-<id>.md` — for verification checks. A project-tier manifest with a `matches:` capability glob, bound to the originating stage by appending its id to that stage's `sensors:` frontmatter list.
...
└── Sensor (two-write install): scaffold a project-tier manifest at
    .aicockpit/sensors/aidlc-<id>.md with a matches: glob, AND append its id to
