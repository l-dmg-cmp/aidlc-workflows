// aidlc-aicockpit-adapter.ts — the aicockpit hook shim (AUTHORED shell file; the
// aidlc-*.ts hook bodies in <project>/.aidlc/hooks/ are PACKAGED core,
// byte-shared with the Claude Code harness).
//
// aicockpit has no settings.json/hooks.json hook registry; its extension seam is
// the PLUGIN API (auto-discovered from .aicockpit/plugin/*.ts, loaded in-process
// by the aicockpit runtime). This one plugin maps aicockpit's hook surface onto
// the core hook bodies, each run as a bun subprocess fed the ClaudeCodeHookInput
// JSON shape the core hooks parse (live-verified on aicockpit 1.17.18):
//
//   aicockpit moment                      → core hook (Claude event it mirrors)
//   ------------------------------------------------------------------------
...
//
// Stop enforcement: session.idle is a REACTIVE event (aicockpit has no blocking
...
export default async ({
  ...
  return {
    ...
    event: async ({ event }: { event: { type: string; properties?: Record<string, unknown> } }) => {
      ...
      // aicockpit provides no stop_hook_active flag and no transcript, so the
      ...
      // Release serialization before the prompt: aicockpit may synchronously
